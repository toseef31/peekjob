...
class IoAdaptorTransport {
	 ...
	 * 
	 */
	public function IoAdaptorTransport($useDomainEnable){
	...
	public function doTrx($msg) {
		...
		if(LogMode::isAppLogable()) {
			$logJournal->writeAppLog("TRY SERVER CONNECT TO : ".$address);
		}
		...
		if(LogMode::isAppLogable()) {
			$logJournal->writeAppLog("SERVER CONNECT OK : ".$address);
		}
